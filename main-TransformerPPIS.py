...
def stack_fn_ddp(batch):
    local_features, all_seq_features, labels = [], [], []
    for i in batch:
        local, seq, label = all_dataSet[i]
...
def main(seed):
    ...
    test_id_idxs = [id_idxs[i] for i in test_list]
    print(len(test_list))
    id_idxs_test = list(set(test_id_idxs))
    print(len(id_idxs_test))  ##70
    # np.random.seed(1)
    ...
    if torch.cuda.is_available() and torch.cuda.device_count() >= 1:
        train_loader = torch.utils.data.DataLoader(train_list, batch_size=batch_size,
                                                ...
                                                num_workers=0, collate_fn=stack_fn_ddp, drop_last=False)
    ...
    file_AUCs = './output/result/output-TransformerPPIS' + '.txt'
    file_model = './output/model/' + 'TransformerPPIS'
...
if __name__ == "__main__":
    ...
    window_size = 0
    all_dataSet = dataSet(window_size, all_encode_file, all_label_file, all_list_file)
